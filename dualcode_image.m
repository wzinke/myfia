function dualcode_image(bckimg, statmap, betamap, sigmap, slcs, sldim, betarng, alpharng, bckrng, ofl)
...
ip = 1;  % interpolation steps (and thus increase image size by this factor)
...
for(i = 1:length(slcs))
    ...
    if(ip > 0)
        ipBCK    = interp2(rot90(squeeze(BCK(x,y,z))),  2,'cubic');      
        ipPE     = interp2(rot90(squeeze(BMAP(x,y,z))), 2,'cubic');       
        alphamap = interp2(rot90(squeeze(ZMAP(x,y,z))), 2,'cubic');      
        ipSIG    = interp2(rot90(squeeze(SMAP(x,y,z))), 2,'cubic');   
    ...
    contour(ipSIG, 1, 'k', 'LineWidth', 1.5);
...
% this function was provided by E. A. Allen with the dualcodeimage code (http://mialab.mrn.org/datavis/)
